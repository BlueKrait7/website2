    ...
    [...Object.keys(Repositories), "algorithms-explanation"].map(
      (repo) =>
        new Promise<void>((resolve, reject) => {
          exec(
            `git clone https://github.com/BlueKrait7/${repo}.git`,
            (err) => {
              if (err) reject(err);
              else resolve();
            }
  ...
  for await (const language of Object.keys(Repositories).filter(
    ...
    for await (const dir of walk(path.join(language, repo.baseDir))) {
      ...
      algorithms[nName].implementations[language] = {
        ...
        url: `https://github.com/BlueKrait7/${language}/tree/master/${path.join(
          ...
          if (
            ...
            if (algorithmMatch) {
              ...
              const dir = algorithmMatch[2].replace(
                "https://github.com/BlueKrait7/C-Sharp/blob/master/",
                ""
              );
              if (!algorithms[nName]) {
                algorithms[nName] = {
              ...
              algorithms[nName].implementations["c-sharp"] = {
                dir,
                url: path.join(
                  "https://github.com/BlueKrait7/C-Sharp/tree/master",
                  algorithmMatch[2]
                ),
          ...
          if (match) {
            ...
            if (algorithm) {
              algorithm.explanationUrl[
                locale.code
              ] = `https://github.com/BlueKrait7/Algorithms-Explanation/tree/master/${dir}`;
